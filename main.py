...
a_c_v = int(input("count of articles to find words: "))
a_c_v = a_c_v if a_c_v < 8 else 7 
a_sets = list(combinations(articles_list, r=a_c_v))
...
if __name__ == "__main__":
    ...
    mfa = {a:ClassAssignment.set_count_loop(ClassAssignment.articles[a]) for a in articles_list}
    for i, v in progress.bar(dicted_a_sets.items()):
        ...
        combinations_from_x[len(temp)] = [v, temp]
    best_match, words_m = combinations_from_x[max(combinations_from_x.keys())]
